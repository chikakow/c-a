...
export class RefundchecksreportComponent implements OnInit {
    ...
    viewAllJournal() {
        ...
        this.model = {
            'modalTitle': (this.atsI18nService.getLocalizedString(this.siteData, 'refundsmanagement.ReportRefundsViewAllJournal')),
            'token': 'refundsmanagement.refundsjournal',
            'hideFromToColumns': true
        }
    }
    ...
    viewJournal(dataItem) {
        ...
        this.modalOpened = true;
        this.model = {
            'parameters': [dataItem.account, dataItem.checkNo],
            'modalTitle': (this.atsI18nService.getLocalizedString(this.siteData, 'refundsmanagement.ReportRefundsJournal', { reportrefund: dataItem.customerName })),
            'token': 'refundsmanagement.reportrefundsjournal',
            'hideFromToColumns': true
        }
    };
    ...
    fillDropdowns() {
        ...
        this.sourceFilterValue = Object.keys(this.sourceFilterValue).map
            (key =>
                ({
                    text: key,
                    value: this.sourceFilterValue[key]
                }));
        ...
        this.daysFilterValue = Object.keys(this.daysFilterValue).map
            (key =>
                ({
                    text: key,
                    value: this.daysFilterValue[key]
                }));
        this.daysFilter = this.daysFilterText[3];
        this.daysFilterCheck = 3;
        this.site = this.siteData.i18n.site;
        this.i18n = this.siteData.i18n.refundsmanagement;
        this.subscription = this.refundsService.getcompany().subscribe((data: any) => {
            this.companyName = data.text;
        });
        this.node = this.siteData.enums.refundsmanagement.NodeEnum.RefundChecksReport;
        let I18nJson: any = {
            "site": this.site,
    ...
    sourceFilterConsumer(text, value) {
        ...
        this.userPreferences = this.siteService.getAngularJson().atsServerValues.userPreferences.refundsmanagement;
        ...
        if (text == 'Date Range') {
            this.fromDateSelectedValue = new Date(
                this.fromDateSelectedValue.getFullYear(),
                this.fromDateSelectedValue.getMonth(),
                this.fromDateSelectedValue.getDate());
            this.fromDate = this.fromDateSelectedValue.toISOString().split('T')[0];
            this.toDateSelectedValue = new Date(
                this.toDateSelectedValue.getFullYear(),
                this.toDateSelectedValue.getMonth(),
                this.toDateSelectedValue.getDate());
            this.toDate = this.toDateSelectedValue.toISOString().split('T')[0];
    ...
    getLocalPreference(moduleName, prefName) {
        let group = this.userPreferences;
        if (!group) {
    ...
    filterReportRefunds() {
        ...
        this.toDate = this.toDateSelectedValue.toISOString().split('T')[0];
        this.isOverlayBusy = true;
        this.subscription = this.refundsService.getreportrefundsdata(
            this.displayingData,
            this.fromDate,
            this.toDate)
            .subscribe((resp: any) => {
